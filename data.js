...
import * as mfx from './myfxbook.js';
import { buildHome, buildAccount } from './engine.js';

const TTL = 5 * 60e3;               // 5 minuti: evita di ripetere il login a ogni ricarica
let cache = { t: 0, conti: null };

/* Nei Worker non esiste process.env: le variabili arrivano nel parametro env
   della richiesta. Si depositano qui una volta e valgono per tutto il giro. */
let ambiente = {};
function impostaAmbiente(env) { ambiente = env || {}; }

function credenziali() {
  const email = ambiente.MYFXBOOK_EMAIL;
  const password = ambiente.MYFXBOOK_PASSWORD;
...
export { home, account, conti, dimostrativi, credenziali, diagnostica, campi,
  impostaAmbiente };
